...
func (r *rpcServer) ListTransfers(ctx context.Context,
	...
	for _, parcel := range parcels {
		...
		for i, delta := range parcel.AssetSpendDeltas {
			...
			var assetID asset.ID
			if senderProof.Asset != nil {
				assetID = senderProof.Asset.ID()
			} else if len(senderProof.ExclusionProofs) > 0 {
				exclusionProof := senderProof.ExclusionProofs[0]
				cProof := exclusionProof.CommitmentProof
				aProof := cProof.AssetProof
				assetID = aProof.AssetID
...
func (r *rpcServer) FundVirtualPsbt(ctx context.Context,
	...
	case in.GetPsbt() != nil:
		...
		_, err = r.cfg.AssetWallet.FundPacket(ctx, desc, vPkt)
		if err != nil {
	...
	case in.GetRaw() != nil:
		...
		vPkt, _, err = r.cfg.AssetWallet.FundAddressSend(ctx, *addr)
		if err != nil {
