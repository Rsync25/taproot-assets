...
func testPsbtInteractiveFullValueSend(t *harnessTest) {
	...
	vPkt := taropsbt.ForInteractiveSend(
		...
		bobAnchorInternalKeyDesc.PubKey, chainParams,
	)
	...
	signResp, err := alice.SignVirtualPsbt(ctxb, &wrpc.SignVirtualPsbtRequest{
		FundedPsbt: fundResp.FundedPsbt,
	})
	require.NoError(t.t, err)

	t.Logf("Signed PSBT: %v",
		base64.StdEncoding.EncodeToString(signResp.SignedPsbt))
	...
	t.Logf("Exporting proof for bob script key %x",
		bobScriptKey.PubKey.SerializeCompressed())
	_ = sendProof(
	...
	assetsJSON, err = formatProtoJSON(bobAssets)
	require.NoError(t.t, err)
